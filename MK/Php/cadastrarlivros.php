    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (isset($_FILES["cover_image"]) && $_FILES["cover_image"]["error"] == 0) {
            ...
            if (in_array($imageFileType, $valid_extensions)) {
                ...
                // Adiciona capítulo se estiver disponível
                if (!empty($_POST['chapter_title']) && !empty($_POST['chapter_content'])) {
                    $chapter_title = $_POST["chapter_title"];
                    $chapter_content = $_POST["chapter_content"];

                    // Insira os dados do capítulo na tabela 'capitulos'
                    $stmt = $pdo->prepare("INSERT INTO capitulos (livro_id, titulo, conteudo) VALUES (?, ?, ?)");
                    $stmt->execute([$livro_id, $chapter_title, $chapter_content]);
                }
...
<body>
    <div class="container">
        ...
        <form method="post" enctype="multipart/form-data">
            ...
            <label for="chapter_title">Título do Capítulo:</label>
            <input type="text" id="chapter_title" name="chapter_title">

            <label for="chapter_content">Conteúdo do Capítulo:</label>
